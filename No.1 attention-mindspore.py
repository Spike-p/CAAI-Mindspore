...
class PreActResidualUnit(nn.Cell):
...
class AttentionModule1(nn.Cell):
    ...
    def __init__(self, in_channels, out_channels, p=1, t=2, r=1):
        ...
        self.sigmoid = nn.SequentialCell(
    ...
    def forward(self, x):
        ...
        x_s = nn.MaxPool2d(x, kernel_size=3, stride=2, padding=1)
        ...
        x_s = nn.ResizeBilinear(x_s, size=shape2)
        x_s += shortcut_short

        #second upsample out 28
        x_s = self.soft_resup3(x_s)
        x_s = nn.ResizeBilinear(x_s, size=shape1)
        x_s += shortcut_long

        #thrid upsample out 54
        x_s = self.soft_resup4(x_s)
        x_s = nn.ResizeBilinear(x_s, size=input_size)
...
class AttentionModule2(nn.Cell):
...
class AttentionModule3(nn.Cell):
...
class Attention(nn.Cell):
    ...
    def __init__(self, block_num, class_num=100):
        ...
        super().__init__()
        self.pre_conv = nn.SequentialCell(
        ...
        self.avg = mindspore.ops.AdaptiveAvgPool2D(1)
        self.linear = nn.Dense(2048, 100)
